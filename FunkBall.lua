...
function run_reverse_spokes(model)
	...
	local s = create_segments_from_vertices(points_on_spokes)
	local intersect_obj = polygon_intersection(vertices, segments, points_on_spokes, s, model)
...
function run_reverse_without_spokes(model)
    ...
    model:creation("spokes and reverse funk ball", ipe.Group({intersect_obj, ipe.Path(model.attributes, { shape })}) )
end
